...
public class GeometryBlobReader {
	...
	private Geometry readGeometry(ByteOrderDataInStream dis, int geometryType,
			boolean hasZ, boolean hasM, boolean compressed) throws IOException {
	...
	private GeometryCollection readGeometryCollection(ByteOrderDataInStream dis,
			boolean hasZ, boolean hasM, boolean compressed) throws IOException {
	...
	private MultiPolygon readMultiPolygon(ByteOrderDataInStream dis, boolean hasZ,
			boolean hasM, boolean compressed) throws IOException {
	...
	private MultiLineString readMultiLineString(ByteOrderDataInStream dis, boolean hasZ,
			boolean hasM, boolean compressed) throws IOException {
	...
	private MultiPoint readMultiPoint(ByteOrderDataInStream dis, boolean hasZ,
			boolean hasM, boolean compressed) throws IOException {
	...
	private Polygon readPolygon(ByteOrderDataInStream dis, boolean hasZ,
			boolean hasM, boolean compressed) throws IOException {
	...
	private Point readPoint(ByteOrderDataInStream dis, boolean hasZ,
			boolean hasM, boolean compressed) throws IOException {
		
		if (compressed) {
			throw new IOException(
					new IllegalArgumentException("Point cannot be compressed."));
		}
	...
	private LineString readLinestring(ByteOrderDataInStream dis, boolean hasZ,
			boolean hasM, boolean compressed) throws IOException {
	...
	private Coordinate readDiffCoordinate(ByteOrderDataInStream dis,
			boolean hasZ, boolean hasM, Coordinate p0) throws IOException {
		...
		if (hasM) {
			...
			MCoordinate cc = new MCoordinate(c);
			cc.m = m;
			c = cc;
	...
	private Coordinate readCoordinate(ByteOrderDataInStream dis, boolean hasZ,
		...
		if (hasM) {
			...
			double m = dis.readDouble();
			MCoordinate cc = new MCoordinate(coordinate);
			cc.m = m;
			coordinate = cc;
